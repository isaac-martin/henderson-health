...
function health_assets_header() {
  ...
  $file = 'x-header';
  wp_enqueue_script( 'health-header', get_stylesheet_directory_uri() . '/js/' . $file . '.js', $deps = array('jquery'), filemtime( get_template_directory() . '/js/' . $file . '.js' ), false );

  // Register and enqueue our main stylesheet with versioning based on last modified time
  wp_register_style( 'health-style', get_stylesheet_uri(), $dependencies = array(), filemtime( get_template_directory() . '/style.css' ) );
  wp_enqueue_style( 'health-style' );
...
function health_assets_footer() {

  // Initialize variables
  $name = 'health-footer';   // This is the script handle
  ...
  if ( health_SCRIPTS_PAGELOAD && ( is_archive() || is_home() || is_search() ) ) {
...
function health_assets_singular() {
  if ( !is_singular() )
    return;
...
add_action( 'wp_enqueue_scripts', 'health_assets_singular' );
